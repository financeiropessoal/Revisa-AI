  ...
  return (
    <div className="p-6 max-w-5xl mx-auto w-full animate-in fade-in slide-in-from-right-4">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
            <h2 className="text-3xl font-bold text-slate-800">Painel de Estatísticas</h2>
            <p className="text-slate-500 mt-1">Acompanhe seu rendimento e constância</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex items-center gap-4">
            ...
            <div>
                <p className="text-sm text-slate-500 font-medium">Revisados Hoje</p>
                <h3 className="text-3xl font-bold text-slate-800">{stats.todayCount}</h3>
            </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex items-center gap-4">
            ...
            <div>
                <p className="text-sm text-slate-500 font-medium">Total de Revisões</p>
                <h3 className="text-3xl font-bold text-slate-800">{stats.totalReviews}</h3>
            </div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-lg font-bold text-slate-800">Desempenho (Últimos 7 dias)</h3>
        </div>

        <div className="h-64 w-full flex items-end justify-between gap-2 md:gap-4 relative pt-6">
            {/* Grid lines background (simple) */}
            <div className="absolute inset-0 flex flex-col justify-between pointer-events-none pb-8 text-xs text-slate-300">
                 <div className="border-b border-slate-100 w-full flex items-end"><span>{stats.maxDaily}</span></div>
                 <div className="border-b border-slate-100 w-full flex items-end"><span>{Math.round(stats.maxDaily / 2)}</span></div>
                 <div className="border-b border-slate-100 w-full flex items-end"><span>0</span></div>
            </div>
            ...
            {stats.last7Days.map((day, idx) => {
                ...
                return (
                    <div key={idx} className="flex-1 flex flex-col items-center justify-end h-full z-10 group relative">
                        ...
                        {/* Bar */}
                        <div 
                            className="w-full max-w-[40px] bg-primary rounded-t-md hover:bg-secondary transition-all duration-500 ease-out"
                            style={{ height: `${Math.max(heightPercentage, 2)}%` }} // min height 2% for visibility
                        ></div>
                        
                        {/* Label */}
                        <div className="mt-2 text-xs text-slate-500 font-medium text-center uppercase tracking-tighter">
                            {day.label}
                        </div>
                    </div>
                );
